...
- **Shift+Tab permission-mode cycler** — a Claude Code-style footer beneath the input shows the active permission mode, and **Shift+Tab** cycles it: **normal** (mutating tools prompt) → **accept edits** (file edits auto-approve, shell still prompts) → **plan** (read-only; propose a plan) → back to normal. **Bypass permissions** (via `/trust on`) shows in the footer too, and Shift+Tab steps back out of it. The modes are a view over the existing permission profiles (`ask`/`edit`/`safe`/`full`) and the `/plan` flag — `/plan`, `/trust`, and `/permissions` continue to work and stay in sync. Cycling is a no-op mid-turn (finish or Ctrl+C first).
