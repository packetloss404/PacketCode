...
- **No public listener, ever, in Phases 1–2.** Transport is loopback or
  SSH-forwarded loopback only.
...
| **PCMP3** | Registry write commands | `/computers register`, `/computers ssh`, and `/computers remove` with validation, duplicate-name refusal, and confirmation before removal. | Command tests incl. invalid input and duplicate names | PCMP1 | queued |
...
| **PCMP7** | Runtime backend abstraction | `RuntimeBackend` (resolve/read/write/execute) with `LocalBackend` preserving today's behaviour exactly, plus `ComputerBackend` forwarding to a daemon. Tools gain no remote conditionals. | Backend parity suite run against both implementations | PCMP6 | queued |
| **PCMP8** | `ComputerID` on jobs | Jobs carry `ComputerID` and `WorkingDir`; `/spawn --computer <name>` routes through the resolved backend; Agent View groups by computer. Approval semantics are unchanged. | Job routing + permission-preservation tests | PCMP7 | queued |
...
PCMP3 is independent of the daemon and can land whenever. PCMP9 is the first
...
Milestone A from `PACKETCOMPUTERS.md`, deliberately data-only. Nothing in
`internal/computers` opens a socket or executes a command.
...
- `internal/app/slashcmd_computers.go` — read-only `/computers` and
...
- packetcode cannot run any work on a registered computer.
- No daemon exists, so no status is ever live.
- Registration is currently done by editing `registry.json` by hand; the
  write commands are PCMP3.
