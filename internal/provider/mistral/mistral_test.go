...
func TestProvider_PricingFallback(t *testing.T) {
	...
	assert.Equal(t, 0.50, in)
	assert.Equal(t, 1.50, out)
	assert.Equal(t, 128_000, p.ContextWindow(DefaultModel))
...
func TestProvider_ListModels_PassesThroughUpstream(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`{"data":[{"id":"codestral-latest"},{"id":"mistral-large-latest"}]}`))
	}))
	defer server.Close()
