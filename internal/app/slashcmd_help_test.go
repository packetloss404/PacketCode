...
func TestHelpReportsResolvedFeatureGateStates(t *testing.T) {
	...
	if got := rig.app.featureGateHelpRows(); got[0].Desc[:8] != "disabled" || got[1].Desc[:8] != "disabled" || got[2].Desc[:8] != "disabled" || got[3].Desc[:8] != "disabled" {
		t.Fatalf("feature states = %#v, want all disabled", got)
	}
}
